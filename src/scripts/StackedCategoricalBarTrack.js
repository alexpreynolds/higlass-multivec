...
const StackedCategoricalBarTrack = (HGC, ...args) => {
  ...
  class StackedCategoricalBarTrackClass extends HGC.tracks.BarTrack {
    ...
    renderTile(tile) {
      ...
      console.log("renderTile", tileX, tileWidth, tile.tileData.tilePos, this.tilesetInfo.tile_size);
    ...
    /**
     * Map each value in every array in the matrix to a color depending on categorical
    ...
    drawVerticalBars(matrix, tileX, tileWidth, positiveMax, negativeMax, tile) {
      ...
      console.log('(categorical) drawVerticalBars:', tileX);
      //console.log('drawVerticalBars:', tileX, trackHeight, positiveMax, tile);
      ...
      const positiveTrackHeight = trackHeight;
      ...
      for (let j = 0; j < matrix.length; j++) { // jth vertical bar in the graph
        ...
        const constantHeight = 1.0;
        const positive = matrix[j][0];
        //console.log("positive", positive);
        //const valueToPixelsPositive = scaleLinear().domain([0, positiveMax]).range([0, positiveTrackHeight]);
        const valueToPixelsPositive = scaleLinear().domain([0, positive.length]).range([0, positiveTrackHeight]);
        let positiveStackedHeight = 0;
        //const height = parseFloat(positiveTrackHeight)/parseFloat(positive.length);
        //console.log("height", height);
        for (let i = 0; i < positive.length; i++) {
          ...
          //graphics.lineStyle(0, 0x0, 0, 0, true);
          positiveStackedHeight = positiveStackedHeight + height;
          if (lowestY > y)
            lowestY = y;
        }
        //console.log("positiveStackedHeight", positiveStackedHeight);
      }
